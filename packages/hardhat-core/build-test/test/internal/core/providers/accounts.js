...
const accounts_1 = require("../../../../src/internal/core/providers/accounts");
const provider_utils_1 = require("../../../../src/internal/core/providers/provider-utils");
const errors_1 = require("../../../helpers/errors");
const mocks_1 = require("./mocks");
function privateKeyToAddress(privateKey) {
    return ethereumjs_util_1.bufferToHex(ethereumjs_util_1.privateToAddress(ethereumjs_util_1.toBuffer(privateKey))).toLowerCase();
}
describe("Local accounts provider", () => {
    ...
    beforeEach(() => {
        ...
        mock.setReturnValue("net_version", provider_utils_1.numberToRpcQuantity(123));
        mock.setReturnValue("eth_getTransactionCount", provider_utils_1.numberToRpcQuantity(0x8));
        mock.setReturnValue("eth_accounts", []);
        wrapper = new accounts_1.LocalAccountsProvider(mock, accounts);
    });
    it("Should return the account addresses in eth_accounts", async () => {
        const response = (await wrapper.request({
            method: "eth_accounts",
        }));
        chai_1.assert.equal(response[0], privateKeyToAddress(accounts[0]));
        chai_1.assert.equal(response[1], privateKeyToAddress(accounts[1]));
    });
    ...
    it("Should throw when calling sendTransaction without gas", async () => {
        ...
        await errors_1.expectHardhatErrorAsync(() => wrapper.request({ method: "eth_sendTransaction", params }), errors_list_1.ERRORS.NETWORK.MISSING_TX_PARAM_TO_SIGN_LOCALLY, "gas");
    });
    it("Should throw when calling sendTransaction without gasPrice", async () => {
        ...
        await errors_1.expectHardhatErrorAsync(() => wrapper.request({ method: "eth_sendTransaction", params }), errors_list_1.ERRORS.NETWORK.MISSING_TX_PARAM_TO_SIGN_LOCALLY, "gasPrice");
    });
    it("Should, given two identical tx, return send the same raw tansaction", async () => {
        ...
        const expectedRaw = "0xf86480830a5c0082520894b5bc06d4548a3ac17d72b372ae1" +
            ...
            "d65383e1ed2e22654";
        chai_1.assert.equal(rawTransaction, expectedRaw);
    });
    it("Should throw if trying to send from an account that isn't local", async () => {
        await errors_1.expectHardhatErrorAsync(() => wrapper.request({
            method: "eth_sendTransaction",
            params: [
                {
        ...
        }), errors_list_1.ERRORS.NETWORK.NOT_LOCAL_ACCOUNT, "0x000006d4548a3ac17d72b372ae1e416bf65b8ead");
    });
    it("Should forward other methods", async () => {
        const input = [1, 2];
        await wrapper.request({ method: "eth_sarasa", params: input });
        chai_1.assert.deepEqual(mock.getLatestParams("eth_sarasa"), input);
    });
    it("Should get the nonce if not provided", async () => {
        ...
        chai_1.assert.equal(mock.getNumberOfCalls("eth_getTransactionCount"), 1);
    });
    describe("eth_sign", () => {
        it("Should be compatible with parity's implementation", async () => {
            ...
            const provider = new accounts_1.LocalAccountsProvider(mock, [
            ...
            chai_1.assert.equal(result, "0x25c349f668c90a890c84aa79a78cf6c74e96483b43ec3ed06aa8aec835477c034aa096e883cc9871aa4ffdffd9f21f6ee4aa4b70f478ad56a18971e4ec2c753e1b");
        });
        it("Should be compatible with ganache-cli's implementation", async () => {
            ...
            chai_1.assert.equal(result.slice(0, -2), "0x84d993fc1b54926db1b6b81544aada29f0f36850a83dc979e8bacfa87e7c7cb11689b2f4ca64697842c42bb7e0cb02dff1851b42e25e62858f27f57bd00ff74b00".slice(0, -2));
        });
        it("Should be compatible with geth's implementation", async () => {
            ...
            chai_1.assert.equal(result, "0x88c6ac158d40e84f519fbb48b6a1355a31202b684163f637fe5c92cc1109acbe5c79a2dd95a8aecff45756c6fc3b4fc8aef345179605bcead2916dd533fb22651b");
        });
        ...
        it("Should just forward if no address is given", async () => {
            ...
            chai_1.assert.deepEqual(mock.getLatestParams("eth_sign"), []);
        });
    });
...
describe("hdwallet provider", () => {
    let mock;
    let wrapper;
    const mnemonic = "couch hunt wisdom giant regret supreme issue sing enroll ankle type husband";
    const hdpath = "m/44'/60'/0'/0/";
    beforeEach(() => {
        mock = new mocks_1.MockedProvider();
        wrapper = new accounts_1.HDWalletProvider(mock, mnemonic, hdpath);
    });
    it("should generate a valid address", async () => {
        const response = (await wrapper.request({
            method: "eth_accounts",
        }));
        chai_1.assert.equal(response[0], "0x4f3e91d2cacd82fffd1f33a0d26d4078401986e9");
    });
    it("should generate a valid address when given a different index", async () => {
        wrapper = new accounts_1.HDWalletProvider(mock, mnemonic, hdpath, 1);
        const response = (await wrapper.request({
            method: "eth_accounts",
        }));
        chai_1.assert.equal(response[0], "0x2a97a65d5673a2c61e95ce33cecadf24f654f96d");
    });
    it("should generate 2 accounts", async () => {
        wrapper = new accounts_1.HDWalletProvider(mock, mnemonic, hdpath, 0, 2);
    ...
    describe("HDPath formatting", () => {
        it("Should work if it doesn't end in a /", async () => {
            wrapper = new accounts_1.HDWalletProvider(mock, mnemonic, "m/44'/60'/0'/0");
            const response = (await wrapper.request({
                method: "eth_accounts",
            }));
            chai_1.assert.equal(response[0], "0x4f3e91d2cacd82fffd1f33a0d26d4078401986e9");
        ...
        it("Should throw if the path is invalid", () => {
            errors_1.expectHardhatError(() => new accounts_1.HDWalletProvider(mock, mnemonic, ""), errors_list_1.ERRORS.NETWORK.INVALID_HD_PATH);
            errors_1.expectHardhatError(() => new accounts_1.HDWalletProvider(mock, mnemonic, "m/"), errors_list_1.ERRORS.NETWORK.INVALID_HD_PATH);
            errors_1.expectHardhatError(() => new accounts_1.HDWalletProvider(mock, mnemonic, "m//"), errors_list_1.ERRORS.NETWORK.INVALID_HD_PATH);
            errors_1.expectHardhatError(() => new accounts_1.HDWalletProvider(mock, mnemonic, "m/'"), errors_list_1.ERRORS.NETWORK.INVALID_HD_PATH);
            errors_1.expectHardhatError(() => new accounts_1.HDWalletProvider(mock, mnemonic, "m/0''"), errors_list_1.ERRORS.NETWORK.INVALID_HD_PATH);
            errors_1.expectHardhatError(() => new accounts_1.HDWalletProvider(mock, mnemonic, "ghj"), errors_list_1.ERRORS.NETWORK.INVALID_HD_PATH);
        });
    });
});
describe("Sender providers", () => {
    ...
    describe("FixedSenderProvider", function () {
        beforeEach(function () {
            wrapper = new accounts_1.FixedSenderProvider(mock, "0x2a97a65d5673a2c61e95ce33cecadf24f654f96d");
        });
        it("Should set the from value into the transaction", async () => {
            ...
            chai_1.assert.equal(params[0].from, "0x2a97a65d5673a2c61e95ce33cecadf24f654f96d");
        });
        it("Should not replace transaction's from", async () => {
            tx.from = "0x000006d4548a3ac17d72b372ae1e416bf65b8ead";
            await wrapper.request({ method: "eth_sendTransaction", params: [tx] });
            const params = mock.getLatestParams("eth_sendTransaction");
            chai_1.assert.equal(params[0].from, "0x000006d4548a3ac17d72b372ae1e416bf65b8ead");
        });
    });
    describe("AutomaticSenderProvider", function () {
        beforeEach(function () {
            wrapper = new accounts_1.AutomaticSenderProvider(mock);
        });
        it("Should use the first account if from is missing", async () => {
            await wrapper.request({ method: "eth_sendTransaction", params: [tx] });
            const params = mock.getLatestParams("eth_sendTransaction");
            chai_1.assert.equal(params[0].from, "0x123006d4548a3ac17d72b372ae1e416bf65b8eaf");
        });
        it("Should not fail on eth_calls if provider doesn't have any accounts", async () => {
            ...
            chai_1.assert.equal(params[0].value, "asd");
        });
        it("Should not replace transaction's from", async () => {
            tx.from = "0x000006d4548a3ac17d72b372ae1e416bf65b8ead";
            await wrapper.request({ method: "eth_sendTransaction", params: [tx] });
